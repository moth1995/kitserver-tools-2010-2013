...
bool GetBinFileName(DWORD afsId, DWORD binId, wchar_t* filename, int maxLen)
{
    ...
    if (!_fast_info_cache[afsId].initialized)
    {
        ...
        if (pBST) 
        {
            ...
            for (it = _info_cache.begin(); it != _info_cache.end(); it++)
            {
                ...
                GetRelativePath(relPath, pBST->relativePathName,
                        sizeof(relPath));
    ...
    TRACE2N(L"Loading afsId=0x%02x, binId=%d", afsId, binId);
    ...
    ZeroMemory(relPath,sizeof(relPath));
    GetRelativePath(relPath, pBST->relativePathName, sizeof(relPath));
...
void InitializeFileNameCache()
{
            ...
            lit++)
    {
        ...
        pattern += L"\\img\\*.img";
        LOG1S(L"processing afs root {%s}",lit->c_str());
        ...
        while(true)
        {
            ...
            if (fData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
            {
                ...
                string key(key_cstr);
                Utf8::free(key_cstr);
                TRACE1S(L"folderpattern = {%s}", folderpattern.c_str());
                        ...
                        if (!(fData1.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) 
                        {
                            ...
                            if (s && swscanf(s+1,L"%d",&binId)==1)
                            {
                                ...
                                if (binId >= 0)
                                {
                                    ...
                                    {
                                        // binID too large
                                        LOG1S1N(L"ERROR: bin ID for filename \"%s\" is too large. Maximum bin ID for this folder is: %d", fData1.cFileName, GetNumItems(folder)-1);
                                    }
...
{	
    ...
    if (s[1]==L':')
    {
        ...
        LOG1S(L"Config: img.dir: {%s}", root.c_str());
    }
    else
    {
        // relative root
            ...
            if (VirtualProtect(bptr, numSongs*sizeof(SONG_STRUCT), newProtection, &protection)) 
            {
                ...
                for (int i=0; i<numSongs; i++)
                {
                    ...
                    if (it != _songs->_songMap.end())
                    {
                        ...
                        LOG1N(L"Set title/artist info for song with binId=%d",ss[i].binId);
                    }
                }
            }
        }
...
bool OpenFileIfExists(const wchar_t* filename, HANDLE& handle, DWORD& size)
{
    ...
    if (handle != INVALID_HANDLE_VALUE)
    {
        ...
        if (k_afs.debug)
            LOG1N(L"OpenFileIfExists: handle = %08x", (DWORD)handle);
        return true;
    }
    return false;
}
...
KEXPORT DWORD afsReadBalls(BALL_INFO* balls)
{
    ...
    DWORD protection = 0;
    DWORD newProtection = PAGE_READWRITE;
    if (VirtualProtect(bptr, NUM_BALLS*sizeof(BALL_INFO), 
                newProtection, &protection)) 
    {
        for (int i=0; i<NUM_BALLS; i++)
        {
            ...
            {
                memcpy(balls[i].name, it->second.name, MAX_BALL_NAME_LEN);
            }
        }
    }
    ...
    for (int i=0; i<NUM_BALLS; i++)
    {
        ...
        LOG1N1S(L"ball #%d: {%s}", i+1, ballName);
        Utf8::free(ballName);
    }
    return 0;
}
