...
techfitModels = [35,36,37,38,39,40,41]
maxModel = 1000
    ...
    def repaint(self, event):
        ...
        if os.path.exists(font):
            ...
            if nameShow == "1":
                ...
                if type == "type2": img = img.Rotate(math.pi/18,wx.Point(27,p[stp]["font-height"]-1))
                elif type == "type3": img = img.Rotate(math.pi/12,wx.Point(27,p[stp]["font-height"]-1))
                elif type == "type4": img = img.Rotate(math.pi/6,wx.Point(27,p[stp]["font-height"]-1))
                bmp = img.ConvertToBitmap()
                x, y = p[stp]["nameA-%s" % type]
                ...
                x, y = p[stp]["nameB-%s" % type]
                y -= int((nameY-24)*p[stp]['name-y-mult'])
                dc.DrawBitmap(bmp, x, y, True)
                bmp = wx.Bitmap(font).GetSubBitmap(rect3)
                img = bmp.ConvertToImage().ResampleBicubic(p[stp]["font-width"],p[stp]["font-height"])
                if type == "type2": img = img.Rotate(-math.pi/18,wx.Point(0,p[stp]["font-height"]-1))
                elif type == "type3": img = img.Rotate(-math.pi/12,wx.Point(0,p[stp]["font-height"]-1))
                elif type == "type4": img = img.Rotate(-math.pi/6,wx.Point(0,p[stp]["font-height"]-1))
                bmp = img.ConvertToBitmap()
                x, y = p[stp]["nameC-%s" % type]
                y -= int((nameY-24)*p[stp]['name-y-mult'])
                dc.DrawBitmap(bmp, x, y, True)
...
for Kitserver 11. Provides simple visual interface to
    ...
    def saveChanges(self, showConfirmation=True):
        ...
        for kit in self.modified.keys():
            ...
            try:
                ...
                cmt = "# Attribute configuration file auto-generated by GDB Manager 11 (%s)" % VERSION
                print >>att, cmt
                print >>att
