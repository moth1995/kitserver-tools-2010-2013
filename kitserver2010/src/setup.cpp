...
#define DEFAULT_EXE_NAME L"pes2010.exe\0"
#define DEFAULT_DEMO_EXE_NAME L"pes2010.exe\0"
#define BUFLEN 4096
...
void MyMessageBox(wchar_t* fmt, DWORD value)
{
	...
	MessageBox(hWnd, buf, L"KitServer 2010 DEBUG MyMessage", 0);
#endif
}
...
%s.", fileName);
			...
			if (dataOffset != 0) {
				...
				else
				{
					...
					swprintf(buf, L"\
======== INFORMATION! =========\n\
KitServer 2010 is already installed (1) for\n\
%s.", fileName);
			...
			if (codeOffset != 0) {
				...
				else
				{
					...
					swprintf(mbuf, L"\
======== INFORMATION! =========\n\
KitServer 2010 is already installed (2) for\n\
%s.", fileName);
	        ...
	        */
			...
			swprintf(buf, L"\
======== SUCCESS! =========\n\
Setup has installed KitServer 2010 for\n\
%s.", fileName);
		...
		else
		{
			...
			swprintf(buf, L"\
======== ERROR! =========\n\
Setup failed to install KitServer 2010 for\n\
%s.\n\
\n\
		...
		if (f != NULL)
		{
			...
			if (dataOffset != 0) {
				...
				if (newEntryPoint == 0)
				{
					...
					swprintf(buf, L"\
======== INFORMATION! =========\n\
KitServer 2010 is not installed for\n\
%s.", fileName);
	        ...
	        */
			...
			swprintf(buf, L"\
======== SUCCESS! =========\n\
Setup has removed KitServer 2010 from\n\
%s.", fileName);
		...
		else
		{
			...
			swprintf(buf, L"\
======== ERROR! =========\n\
Setup failed to remove KitServer 2010 from\n\
%s.\n\
\n\
(No modifications made.)\n\
Verify that the executable is not\n\
