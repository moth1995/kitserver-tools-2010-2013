...
HRESULT STDMETHODCALLTYPE initModule(IDirect3D9* self, UINT Adapter,
	...
	if(readfile(filename.c_str(), nationals))
	{
		...
		if(nationals != NULL)
			LOG(L"Found %d national leagues", _msize(nationals)/sizeof(nationals[0]));
		else
	...
	if(readfile(filename.c_str(), clubs))
	{
		...
		if(clubs != NULL)
			LOG(L"Found %d club leagues", _msize(clubs)/sizeof(clubs[0]));
		else
...
bool readfile(const wchar_t* cfgFile, _league* &theleagues)
{
	...
	while (!feof(f))
	{
        ...
        WORD numKey;
		count++;
		if (swscanf(pName, L"%hd", &numKey)==1)
        {
            while (*pValue == ' ')
                pValue++;
