...
void GDB::load()
{
    ...
    for (unordered_map<WORD,wstring>::iterator it = mapFile.begin(); it != mapFile.end(); it++)
    {
        ...
        {
            LOG1N1S(L"ERROR: Unable to find or read folder for team %d (%s). Skipping it.", it->first, dirName.c_str());
            continue;
        }
...
void GDB::fillKitCollection(KitCollection& col, int kitType)
{
	...
	{
		// none found.
        LOG1S(L"WARNING: no kit folders found for search pattern: {%s}", pattern.c_str());
		return;
	}
	while(true)
	{
